...
The artifact upload feature enables Continued agents running in devboxes to upload arbitrary files (screenshots, videos, logs) to agent session storage for review and debugging purposes. The architecture uses a two-step presigned URL pattern for secure, performant uploads.
...
These are automatically provided when running in Continued's devbox environment.
