...
Workspace or shared-assistant MCP servers that use `stdio` are additionally
held until explicitly approved in the MCP selector. For unattended runs, only
opt into starting those servers when the configuration is trusted:

```bash
CONTINUE_ALLOW_UNAPPROVED_MCP=true cn -p "Inspect the configured services"
```

This opt-in launches the configured `command` with its `args`, environment, and
working directory; keep it disabled for repositories or assistants you do not
trust.
