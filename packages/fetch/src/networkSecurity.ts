...
/** Validate a URL before a public-only request is made. */
...
/**
 * Resolve a hostname at connection time and only hand public addresses to the
 * HTTP client. This prevents a DNS answer from changing between validation and
 * socket creation (DNS rebinding).
 */
export const publicDnsLookup: LookupFunction = (
  hostname,
  options,
  callback,
) => {
  dnsLookup(hostname, { all: true, verbatim: true }, (error, addresses) => {
    ...
    const publicAddresses = addresses.filter(
      ({ address }) => !isPrivateNetworkAddress(address),
    );
    if (publicAddresses.length === 0) {
      callback(
        new Error("The URL resolves to a private or local network address"),
        "",
        0,
      );
      return;
